...
getPublicKeyUrl="https://jwxt.stbu.edu.cn/xtgl/login_getPublicKey.html?time=" # 获取公钥
getocrdddUrl="https://jwxt.stbu.edu.cn/kaptcha?time="
loginUrl="https://jwxt.stbu.edu.cn/xtgl/login_slogin.html?time="
getSessinUrl="https://jwxt.stbu.edu.cn/xtgl/login_slogin.html?time="
def getProxy():
    proxy_list=[]
...
def getPublicKey(session):
    ...
    url=getPublicKeyUrl+str(start_time)+"&_="+str(end_time)
    print(url)
    Proxy=getProxy()
...
def ocrddd(session):
    today = datetime.datetime.now()  # 获取今天时间
    start_time = int(round(today.timestamp()*1000))
    url=getocrdddUrl+str(start_time)
    Proxy=getProxy()
    response=session.get(url=url,headers=headers,proxies=Proxy)
...
def getSession():
    ...
    start_time = int(round(today.timestamp() * 1000))
    Proxy=getProxy()
    url=getSessinUrl+str(start_time)
